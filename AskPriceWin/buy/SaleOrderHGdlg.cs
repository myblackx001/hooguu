...
{
    public partial class SaleOrderHGdlg : Form
    {
        ...
        private void SaleOrderHGdlg_Load(object sender, EventArgs e)
        {
            textBox2.Text = oldorderid;
            comboBox1.Text = username;
            richTextBox1.Text = remark;
        }

        private void SaleOrderHGdlg_SizeChanged(object sender, EventArgs e)
        {
            int w = this.Width;
            int h = this.Height;
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            {
                conn.Open();
                string sql = "update extraorderlist set isorder = 1,warehouse =concat(warehouse , '" + warehouseID + ", ' ) where orderid = '"+oldorderid+"'";
                using (MySqlCommand comm = new MySqlCommand(sql, conn))
                {
                    comm.ExecuteReader();
